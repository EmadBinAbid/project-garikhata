    ...
    for shape in count_shapes:
        if(shape[0] in shapes_list):
            ...
            html_function_init = html_function_init + '\t\t\t\tfor(var ' + shape[0] + 'number=0; ' + shape[0] + 'number<' + str(shape[1]) + '; ' + shape[0] + 'number++)\n' \
                                                ...
                                                '\t\t\t\t}\n'
    html_function_shape = ""
    html_function_shape = html_function_shape + '\t\t\t\tcount = count + 1;\n'
    html_function_shape = html_function_shape + '\t\t\t\tif (count%2 == 0)\n'
    html_function_shape = html_function_shape + '\t\t\t\t{\n'
    html_function_shape = html_function_shape + '\t\t\t\t\twindow.location.reload(false);\n'
    html_function_shape = html_function_shape + '\t\t\t\t}\n'
    html_function_shape = html_function_shape + '\t\t\t\telse\n'
    html_function_shape = html_function_shape + '\t\t\t\t{\n'
